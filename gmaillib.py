...
class message:
    def __init__(self, fetched_email):
		...
		parsed = email.message_from_string(fetched_email.decode(errors='ignore'))
...
class account:
	...
	def inbox(self,N=10):
		...
		fetch_list = fetch_list.decode(errors='ignore').split(' ')[-N:]
		inbox_emails = []
	...
	def unread(self):
		...
		fetch_list = fetch_list.decode(errors='ignore').split(' ')
		if fetch_list == ['']:
			return []
		unread_emails = []
		for each_email in fetch_list:
	...
	def get_email(self, email_id):
		self.recieveserver.select('Inbox')
		#This syntax fetches the email as a string
